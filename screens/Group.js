...
const Group = () => {
    ...
    const handleCreateGroup = () => {
        ...
        const usersToAdd = users
            .filter(user => selectedItems.includes(user.id)) // Get selected users
        ...
        setDoc(newRef, {
            ...
            users: usersToAdd, // Add all selected users and the current user
            messages: [{
                sender: auth?.currentUser?.email, // Sender is the current user
                message: `Welcome to the group: ${groupName}`, // Welcome message
                timestamp: Date.now(),
            }],
            groupName: groupName,
            groupAdmins: [auth?.currentUser?.email] // Set the current user as the admin
        }).then(() => {
